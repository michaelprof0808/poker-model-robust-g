...
class Miner(BaseMinerNeuron):
    ...
    # BaseNeuron marks these legacy names abstract. The v3 Axon attaches the
    # micro-session-specific methods above, but concrete aliases keep the class
    # instantiable across Bittensor/ABC implementations.
    async def forward(self, synapse: Any) -> MicroSessionDetectionSynapse:
        return await self.forward_micro_sessions(synapse)

    async def blacklist(self, synapse: Any) -> Tuple[bool, str]:
        return await self.blacklist_micro_sessions(synapse)

    async def priority(self, synapse: Any) -> float:
        return await self.priority_micro_sessions(synapse)
